...
def send_sticker(sender, sticker_id):
    authorize.method('messages.send', {'user_id': sender, 'sticker_id': sticker_id, 'random_id': get_random_id()})
...
def num_to_word(a):
    ...
    for i in range(0, n):
        ans += ((numb_names[0 if (n - i - 1) % 3 != 1 else 1][lst[i]] + numb_suffixes[n - 1 - i]) if lst[i] else "") + " "
    ans = ans.rstrip()
    return ans
...
for event in longpoll.listen():
    if event.type == VkEventType.MESSAGE_NEW and event.to_me and event.text:
        ...
        if received_message == '!change':
            ind1, ind2 = ind2, ind1
            write_message(sender, "Текущий режим: с " + rus_bur[ind1] + " на " + rus_bur[ind2])  
            continue
        flg = 0
        ...
        if len(words) == 0:
            write_message(sender, "Я вас не понимаю... Скорее всего, этого слова нет в словаре")
